...
pub fn build(b: *std.Build) !void {
    const preverified_generated = b.option(
        bool,
        "proof-preverified-generated",
        "Consume proof-generated modules that were verified before this build",
    ) orelse false;
    ...
    const materialize = b.step(
        ...
        "Materialize generated proof inputs without compiling the helper",
    );
    const help_strings = bindGeneratedModule(b, exe, materialize, preverified_generated, "help_strings", "help_strings.zig");
    ...
    const wuffs_c = bindGeneratedModule(b, exe, materialize, preverified_generated, "wuffs_c", "wuffs_c.zig");
...
fn bindGeneratedModule(
    ...
    const module = found orelse @panic("proof generated module is missing");
    const source = module.root_source_file orelse @panic("proof generated module has no source");
    const producer = switch (source) {
        .generated => |generated| generated.file.step,
        else => @panic("proof generated module source is not generated"),
    };

    const destination = b.fmt("proof-generated/{s}", .{filename});
    if (!preverified) {
...
fn detachDirectDependency(step: *std.Build.Step, dependency: *std.Build.Step, expected: usize) void {
    ...
    if (removed != expected) @panic("proof generated module dependency count mismatch");
}
...
fn assertProducerDetached(b: *std.Build, root: *std.Build.Step, producer: *std.Build.Step) void {
    ...
    if (stepReaches(b, root, producer, &seen)) {
        @panic("proof generated module producer remains in final graph");
    }
}
    ...
    for (module.import_table.keys(), module.import_table.values()) |name, imported| {
        if (std.mem.eql(u8, name, import_name)) {
            if (found.* != null and found.* != imported) {
                @panic("proof generated module is ambiguous");
            }
            found.* = imported;
        }
